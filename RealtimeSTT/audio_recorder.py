        ...
        while not self.shutdown_event.is_set():
            if self.conn.poll(0.01):
                try:
                    ...
                    logger.error(f"Error receiving data from connection: {e}", exc_info=True)
            else:
            ...
            while not self.shutdown_event.is_set():
                try:
                    ...
                    try:
                        ...
                        logger.debug(f"Final text detected with main model: {transcription}")
                    ...
                    logger.debug("Transcription worker process finished due to KeyboardInterrupt")
                    break
                except Exception as e:
                    logger.error(f"General error in processing queue item: {e}", exc_info=True)
        finally:
        ...
        def initialize_audio_stream(audio_interface, sample_rate, chunk_size):
            ...
            while not shutdown_event.is_set():
                try:
                    ...
                    logger.info(f"Microphone connected and validated (input_device_index: {input_device_index})")
                    return stream
                ...
                for rate in sample_rates_to_try:
                    try:
                        ...
                        if stream is not None:
                            logger.debug(f"Audio recording initialized successfully at {device_sample_rate} Hz, reading {chunk_size} frames at a time")
                            # logger.error(f"Audio recording initialized successfully at {device_sample_rate} Hz, reading {chunk_size} frames at a time")
                            return True
                    except Exception as e:
                        logger.warning(f"Failed to initialize audio23 stream at {device_sample_rate} Hz: {e}")
                        continue
