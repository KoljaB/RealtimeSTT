...
| Milestone 3: wake-word runtime | `tests/unit/test_wakeword_dependencies.py`, `tests/unit/test_fastapi_server_multi_user.py` | Dependency loading and server flow are covered. Runtime Porcupine/OpenWakeWord scoring remains weak. |
...
- Target: `RealtimeSTT/_audio_recorder/transcription.py`.
...
```powershell
python -c "from pathlib import Path; [compile(Path(p).read_text(encoding='utf-8'), p, 'exec') for p in ['RealtimeSTT/audio_recorder.py', 'RealtimeSTT/_audio_recorder/transcription.py']]; print('compile ok')"
